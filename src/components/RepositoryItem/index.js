...
const RepositoryItem = props => {
  const [apiResponse, setResponse] = useState(differentView.initial)
  ...
  const getApi = useCallback(async () => {
    ...
    if (response.ok) {
      ...
      setResponse(differentView.success)
    } else {
      setResponse(differentView.failure)
    }
  }, [username, repoName]) // Added dependencies
  ...
  }, [username, getApi]) // Added getApi to dependency array
